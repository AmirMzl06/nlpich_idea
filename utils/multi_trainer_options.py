...
from utils.multi_model_options import Encoder_Decoder_Multi
import os
import numpy as np
...
from utils.multi_trainer import get_dataset_loaders_speech_nejm
...
def train_model(args):
    ...
    model = Encoder_Decoder_Multi(
        ...
        pooling=args.get("pooling", False),
        batch_norm=args.get("batch_norm", False),
        normalize=not args.get("ceb_nonorm", False),
    ...
    for batch in trange(args["nBatch"]):
        ...
        for i, (session_name, train_loader) in enumerate(train_loaders):
            ...
            # Clean Forward
                ...
                reference, positive, negative, ref_batch_idx, ref_time_idx, pos_time_idx, neg_batch_idx, neg_time_idx, = get_batch(embeddings, emb_lengths, args['cont_batch'], args['offset'])
