...
from utils.eval_utils import get_dataset_loaders_speech_nejm, get_dataset_loader_nlp_10, get_dataset_loader_nlp_21
...
def get_dataset_loaders(
    ...
    all_loaders = [
        ('speech', 'speech', speech_loader),
        ('nlp10', 'nlp', nlp10_loader),
        ('nlp21', 'nlp', nlp21_loader),
        ('nejm','speech', nejm_dataset),
    ]
    return all_loaders

def eval_model(model, session_name,task_name, test_loader, device='cuda'):
    ...
    with torch.no_grad():
        ...
        for X, y, X_len, y_len, testDayIdx in tqdm(test_loader):
            with torch.autocast("cuda", dtype=torch.bfloat16, enabled=True):
                ...
                pred, lengths = model(X, X_len, session_name, task_name)
                loss = ctc_criterion(
...
def model_logits(model, session_name, task_name, test_loader, device='cuda'):
    model = model.to(device)
