...
def train_model(args : dict):
    ...
    ctc_apply_to_adv = args.get("ctc_apply_to_adv", False)
    ...
    for batch in trange(args["nBatch"]):
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16, enabled=True):
            ...
            reference, positive, negative, ref_batch_idx, ref_time_idx, pos_time_idx, neg_batch_idx, neg_time_idx, = get_batch(embeddings, emb_lengths, args['cont_batch'], args['offset'], False, True, True, all_ref)
        ...
        if adv:
            ...
            for i in range(steps):
                ...
                with torch.autocast("cuda", dtype=torch.bfloat16, enabled=True):
                    ...
                    if i == 0:
                        reference, positive, negative, ref_batch_idx_adv, ref_time_idx_adv, pos_time_idx_adv, neg_batch_idx_adv, neg_time_idx_adv, = get_batch(embeddings_adv, emb_lengths, args['cont_batch'], args['offset'], False, True, True, True)
                    else:
                        reference, positive, negative = embeddings_adv[ref_batch_idx_adv, ref_time_idx_adv], embeddings_adv[
                            ref_batch_idx_adv, pos_time_idx_adv], embeddings_adv[neg_batch_idx_adv, neg_time_idx_adv]

                    negative = negative.detach()
                    positive = positive.detach()
                    loss_contrastive_adv = criterion(reference, positive, negative)[0]
                    loss_adv = loss_contrastive_adv

                grad_ctc = torch.zeros_like(X_adv)


                grad_infonce = torch.autograd.grad(loss_contrastive_adv, X_adv, only_inputs=True)[0]
                grad = grad_ctc + grad_infonce
                with torch.no_grad():
                    if adv_norm == 'linf':
