...
class Encoder_Decoder_Multi(nn.Module):
    ...
    def __init__(self, cebra_out_dim, kernel, stride, rnn_hidden, rnn_layers, rnn_dr = 0.4, rnn_bidir=True, cebra_unfolder=False, gru = False, smooth_width=2.0, gauss_in=True, shared_rnn=False):
        super().__init__()
        def init_cebra():
            return Offset36_multi(256, cebra_out_dim)
    ...
    def add_session_encoder(self, session_name, num_neurons):
    ...
    def add_session_decoder(self, session_name, num_outputs):
        new_dec = RNN_Decoder(
            self.rnn_dim,
            self.gru,
            self.rnn_hidden,
    ...
    def forward(self, x, lengths, session_name, decoder_name):
        ...
        if self.shared_rnn:
            x, _ = self.rnn(x)
        x = self.decoders[decoder_name](x)
        return x, lengths
