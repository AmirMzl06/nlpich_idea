...
def train_model(args):
    ...
    n_steps_freeze = args.get("n_steps_freeze", 3)
    unfreeze = args.get("unfreeze", False)
    task_based = args.get("task_based", False)
    contrastive = args.get("contrastive", False)
    ...
    model.add_session_decoder('nlp', 32)
    ...
    contrastive_criterion = InfoNCE(0.4)
    model.eval()
    ...
    pretrain_folder = f"dim256{args['seed'] if args['seed'] != 0 else ''}"
    if adv: pretrain_folder = "pretrained-adv-0105"
    ...
    # train_iters = [iter(loader) for session_name, dec_name, loader in train_loaders]
    ...
    for batch in trange(args["nBatch"]):
        ...
        for i, (session_name, decoder_name, train_loader) in enumerate(train_loaders):
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16, enabled=True):
                ...
                if contrastive:
                    embeddings, emb_lengths = model.transformer_embs()
                    reference, positive, negative, ref_batch_idx, ref_time_idx, pos_time_idx, neg_batch_idx, neg_time_idx, = get_batch(
                        embeddings, emb_lengths, 256, 1, False, True, True,
                        False)

                    loss_contrastive = contrastive_criterion(reference, positive, negative)[0]
            ...
            if adv:
                epsilon = 0.1 if "nlp" in session_name else 0.03
                steps = 10
                alpha = epsilon / 5.0
                ...
                for _ in range(steps):
                    ...
                    with torch.autocast("cuda", dtype=torch.bfloat16, enabled=True):
                        ...
                        if contrastive:
                            embeddings_adv, emb_lengths_adv = model.transformer_embs()
                            reference, positive, negative = embeddings_adv[ref_batch_idx, ref_time_idx], embeddings_adv[
                                ref_batch_idx, pos_time_idx], embeddings_adv[neg_batch_idx, neg_time_idx]
                            negative = negative.detach()
                            positive = positive.detach()
                            loss_contrastive_adv = contrastive_criterion(reference, positive, negative)[0]
                            ctc_loss_adv = loss_contrastive_adv + ctc_loss_adv
                ...
                with torch.autocast("cuda", dtype=torch.bfloat16, enabled=True):
                    ...
                    if contrastive:
                        ...
                        reference, positive, negative = embeddings_adv[ref_batch_idx, ref_time_idx], embeddings_adv[
                            ref_batch_idx, pos_time_idx], embeddings_adv[neg_batch_idx, neg_time_idx]
                        loss_contrastive_adv = contrastive_criterion(reference, positive, negative)[0]
                        loss_adv = loss_contrastive_adv + loss_adv
