...
from utils.neuron_subset_attack import neuron_subset_pgd, validate_neuron_options
...
def train_model(args : dict):
    ...
    adv_neuron_refresh = args.get('adv_neuron_refresh', 'step')
    adv_neuron_percent = validate_neuron_options(
        args.get('adv_neuron_percent', 100.0), adv_neuron_refresh)
    if adv_neuron_percent < 100 and not adv:
        raise ValueError('Use --adv to enable --adv_neuron_percent below 100.')
    if adv and adv_neuron_percent < 100:
        print(f'Neuron-subset PGD: top {adv_neuron_percent:g}% per sample, '
              f'refresh={adv_neuron_refresh}, norm={adv_norm}; padding excluded.', flush=True)
    ...
    for batch in trange(args["nBatch"]):
        ...
        if adv:
            ...
            if adv_neuron_percent < 100:
                # Reuse the clean pass's contrastive sample indices. This is
                # exactly the original inner-attack objective and detach policy.
                def attack_loss(inputs):
                    with torch.autocast("cuda", dtype=torch.bfloat16, enabled=True):
                        pred_probe, probe_lengths = model(inputs, X_len)
                        if isinstance(model, torch.nn.DataParallel):
                            probe_emb, _ = model.module.get_cebra_embs()
                        else:
                            probe_emb, _ = model.get_cebra_embs()
                        probe_ctc = ctc_criterion(
                            torch.permute(pred_probe.log_softmax(2), [1, 0, 2]),
                            y, probe_lengths, y_len,
                        ).sum()
                        probe_ref = probe_emb[ref_batch_idx, ref_time_idx]
                        probe_pos = probe_emb[ref_batch_idx, pos_time_idx].detach()
                        probe_neg = probe_emb[neg_batch_idx, neg_time_idx].detach()
                        return criterion(probe_ref, probe_pos, probe_neg)[0] + probe_ctc

                X_adv, _neuron_mask = neuron_subset_pgd(
                    X, X_len, attack_loss, epsilon, adv_norm,
                    adv_neuron_percent, refresh=adv_neuron_refresh,
